...
const ChooseWordBySynonym = () => {
  ...
  return (
    ...
    >
      <div className="quiz-challenges-result-name">Choose word by synonym</div>
      ...
      <div></div>
      {challenges
        .filter((c) => c.__type === QuestionType.ChooseWordBySynonym)
        .map((q) => (
          <React.Fragment key={`${q.wordId}_cws`}>
            <div>{q.word}</div>
            <div>{q.question}</div>
            <div className="quiz-challenges-options-cell">
              {q.options?.map((o) => (
                <div key={`${q.wordId}_options_cws_${o}`}>{o}</div>
              ))}
            </div>
            <div>{q.answer}</div>
            <div>
...
export default ChooseWordBySynonym;
